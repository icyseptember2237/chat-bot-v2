...
func (m *ReceiveMessage) RewriteForward() {
	for i := range m.Message {
		msg := m.Message[i]
		if msg.Type == SubForwardMsg {
			var got ReceiveMessage
			req := map[string]int64{
				"message_id": m.MessageId,
			}
			err := bot_api.RequestBot(http.MethodPost, "/get_msg", req, &got)
			if err != nil {
				logger.Info(context.Background(), fmt.Sprintf("RewriteForward err: %v", err))
				return
			}
			for _, gotMsg := range got.Message {
				if gotMsg.Type == SubForwardMsg {
					if id, ok := gotMsg.Data["id"]; ok && msg.Data["id"] == id {
						m.Message[i].Data = gotMsg.Data
					}
				}
			}
		}
